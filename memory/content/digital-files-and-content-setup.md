...
| **`memory/constellation/`** | Constelación del proyecto: notas `00`–`08`, visualizador [`CONSTELACION-COSMICA.html`](../constellation/CONSTELACION-COSMICA.html), portal [`🌌-CONSTELACION-COSMICA.md`](../constellation/🌌-CONSTELACION-COSMICA.md). **Canónico en git** (HTML): commit `9c8fdaf6` — si se hizo `git restore` o se perdió trabajo **sin** commit, recuperar con `git checkout 9c8fdaf6 -- memory/constellation/CONSTELACION-COSMICA.html`; si solo existía en el editor, **Timeline** de Cursor/VS Code. **Convención:** versionar en git si el equipo debe compartir el mapa; Obsidian abre los mismos `.md`. |
