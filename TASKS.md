...
- [ ] **Oleada 3 — marca / UX / comercial / activos:** brief [`memory/research/gano-wave3-brand-ux-master-brief.md`](memory/research/gano-wave3-brand-ux-master-brief.md) + cola [`agent-queue/tasks-wave3.json`](.github/agent-queue/tasks-wave3.json). **Actions → Seed Copilot task queue** → `queue_file: tasks-wave3.json` → `scope: all` (o filtrar `theme` / `content_seo` / `commerce` / `docs` / `coordination`). Luego asignar Copilot + [`.github/prompts/copilot-bulk-assign.md`](.github/prompts/copilot-bulk-assign.md).
